...
const TextDivider = ({ text }) => {
  return (
    <div id="final-divider" className="govuk-radios__divider">
      {text}
    </div>
  );
};
...
class RadioFieldSet extends Component {
  ...
  formSubmit = () => {
    ...
    if (value) {
      ...
      if (selectedOption.conditional) {
        ...
        if (this.conditionalValue[value]) {
          if (
            ...
            return this.setState({
              conditionalError: {
                msg: selectedOption.conditional.invalidInputErrorMessage,
                field: this.getConditionalId(optionIndex),
              },
              activeError: true,
            });
          }
        ...
        return this.setState({
          conditionalError: {
            msg: selectedOption.conditional.emptyInputErrorMessage,
            field: this.getConditionalId(optionIndex),
          },
          activeError: true,
        });
  ...
  hasConditionalError(i) {
    return (
      ...
      this.state.conditionalError.field === this.getConditionalId(i)
    );
  }
      ...
      <div>
        {(this.state.error || this.state.conditionalError) && (
          <ErrorSummary
            ...
            errorSummaryTextAndLocation={[
              {
                text: this.state.conditionalError
                  ? this.state.conditionalError.msg
                  : this.errorText,
                location: `#${this.state.actionableFieldId}`,
              },
            ]}
            pageTitle={`${this.title} - ${serviceName}`}
          />
        )}
        <div
        ...
        >
          ...
          >
            ...
            {this.hintText && (
              <div id={`hint-text-${this.name}`} className="govuk-hint">
                {this.hintText}
              </div>
            )}
            <span id={`${this.name}-error`} className="govuk-error-message">
              {this.state.error}
            </span>
            <div
              className={
                this.conditional ? 'govuk-radios--conditional' : 'govuk-radios'
              }
            >
        ...
        <div>
          <div id="before-button-content">{this.beforeButton}</div>
...
RadioFieldSet.propTypes = {
  ...
  options: PropTypes.arrayOf(
    PropTypes.shape({
      ...
      conditional: PropTypes.objectOf(
        PropTypes.shape({
